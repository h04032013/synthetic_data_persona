...
def main(args):
    ...
    llm = LLM(model=model_path, tensor_parallel_size=1) # please set tensor_parallel_size based on the GPUs you are using

    prompts = []
    #2048 is standard max in in vllm_synthetize.py from personhub repo
    #max_len = 2048 
    max_len = 4096
    ...
    sampling_params = SamplingParams(temperature=0.6, seed =0, top_p=0.95, max_tokens=max_len, stop=["<|eot_id|>"])
